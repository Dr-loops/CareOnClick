    ...
    for (const msg of data.messages) {
        try {
            await prisma.message.create({
                data: {
                    id: msg.id,
                    senderId: msg.sender, // The legacy 'sender' field was name, not ID. This will fail FK.
                    // We need to look up user by name... risky.
            ...
            console.warn(`Skipping message ${msg.id}: Sender ID issue? ${e.message}`);
        }
    }

    // Skip notifications for now due to complexity of recipient mapping
