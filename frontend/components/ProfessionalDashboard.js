...
export default function ProfessionalDashboard({ user }) {
    ...
    const [newTask, setNewTask] = useState({ description: '', patientId: '', dueDate: '', priority: 'Normal' });
    ...
    const handleCreateTask = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                alert('Task created!');
                setNewTask({ description: '', patientId: '', dueDate: '', priority: 'Normal' }); // Reset
                fetchTasks();
            } else {
                        ...
                        {/* Tasks Tab */}
                        {
                            activeLabTab === 'tasks' && (
                                <Card>
                                    ...
                                    <div className="tasks-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                                        <div>
                                            <h4 style={{ marginBottom: '1rem' }}>My Tasks & Follow-Ups</h4>

                                            {/* Add Task Form */}
                                            <div style={{ marginBottom: '1.5rem', padding: '1rem', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                                                <h5 style={{ margin: '0 0 0.5rem 0' }}>+ Add New Task</h5>
                                            ...
                                            {loadingTasks ? <p>Loading tasks...</p> : (
                                                <div style={{ display: 'grid', gap: '1rem', maxHeight: '500px', overflowY: 'auto' }}>
                                                    {tasks.filter(t => t.status !== 'Completed').length === 0 && <p style={{ color: '#888' }}>No pending tasks.</p>}
                                                    {tasks.filter(t => t.status !== 'Completed').map(task => (
                                                        <Card key={task.id} className="task-card" style={{ borderLeft: `4px solid ${task.priority === 'Urgent' ? 'orange' : (task.priority === 'Stat' ? 'red' : 'blue')}` }}>
                                                            <div>
                                                                ...
                                                                <div style={{ fontSize: '0.8rem', color: '#666' }}>
                                                                    Patient: {task.patientName || task.patient?.name || 'Unknown'}
                                                                </div>
                                                                {task.dueDate && <div style={{ fontSize: '0.8rem', color: '#888' }}>Due: {new Date(task.dueDate).toLocaleDateString()}</div>}
                                                            </div>
                                                            <div style={{ marginTop: '0.5rem', textAlign: 'right' }}>
                                                                <Button onClick={() => handleCompleteTask(task.id)} variant="secondary" size="sm">✅ Mark Done</Button>
                                                            </div>
                                                        </Card>
                                                    ))}
                                                </div>
                                        ...
                                        <div>
                                            <h4 style={{ marginBottom: '1rem' }}>Completed Tasks (Recent)</h4>
                                            <div style={{ display: 'grid', gap: '0.8rem', opacity: 0.8 }}>
                                                {tasks.filter(t => t.status === 'Completed').slice(0, 5).map(task => (
                                                    <div key={task.id} style={{ padding: '0.5rem', border: '1px solid #eee', borderRadius: '4px', background: '#f9f9f9' }}>
                                                        <div style={{ textDecoration: 'line-through', color: '#888' }}>{task.description}</div>
                                                        <div style={{ fontSize: '0.75rem', color: '#aaa' }}>{task.patientName} • Completed</div>
                                                    </div>
                                                ))}
                                            </div>

                                            <h4 style={{ marginTop: '2rem', marginBottom: '1rem' }}>Critical Result Alerts</h4>
